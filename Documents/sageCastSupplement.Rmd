...
for(doArea in subDirs) {
   ...
   thiscap <- paste0("Posterior distributions of key parameters for the ", thisname, " core area.")
   ...
   names(alltables)[i] <- paste0("Statistical summaries of posterior distributions of key parameters for the ", thisname, " core area. SD = standard deviation; 2.5% and 97.5% quantiles bound the 95% Bayesian credible interval.")
...
```{r equilibrium-cover, eval = TRUE, fig.cap="Posterior distributions of equilibrium cover calculated from fitted model parameters. The vertical black lines show the observed mean cover for each core area from 1985-2018."}
nameMap <- read.csv("../AuxData/CoreAreaNamesMapping.csv")
