...
for(doCore in cores) {
  thisname <- nameMap %>%
    ...
    pull(Name)
  thiscap <- paste0("Potential scale reduction factors ($\\hat{R}$) for all parameters in the model for the ", thisname, " core area.")
...
knitr::kable(allp, digits = 2, caption = "Bayesian P-values for each core area. P-values greater than 0.95 or less than 0.05 indicate lack of fit. See main text for description of the P-value calculations.")
```
...
for(doArea in subDirs) {
   ...
   allplots[[i]] <- ggplot(tmpDf, aes(x = value, y=..density..)) +
    ...
    ggtitle(doArea)
   names(allplots)[i] <- paste0("Caption ", i)
   # print(p)
...
```{r equilibrium-cover, eval = FALSE}
nameMap <- read.csv("../AuxData/CoreAreaNamesMapping.csv")
...
```{r colonization-res, eval = FALSE, results='asis'}
...
```{r targs,  eval = FALSE, results = 'asis'}
...
```{r nesting-targs, fig.height=6, eval = FALSE}
