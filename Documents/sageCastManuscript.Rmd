...
\noindent{}Code, intermediately derived data, and results are archived on Zenodo [@tredennick_zenodo_2023]. Projections generated during this study are available as a USGS data release [@tredennick_data_2023]. All data used in this study are publicly available elsewhere: sagebrush cover data, @rigge_quantifying_2020; PRISM climate product, http://prism.oregonstate.edu/; CMIP climate projections, @eyring_overview_2016 accessed with **epwshiftr** R package [@jia_epwshiftr_2020].
...
Because populations, communities, and ecosystems are not static, using the current status of an ecosystem may be insufficient to define protected areas for species [@hannah_protected_2007; @monzon_climate_2011] or to make optimal management decisions [@boettiger_optimal_2016].
...
Land management in the western United States focuses on several uses and species, but Greater sage-grouse (*Centrocercus urophasianus*, hereafter "sage-grouse") has historically driven the conversation in sagebrush dominated ecosystems.
...
[USFWS] 2010).
A further review of population status and conservation efforts concluded the species did not warrant listing in 2015 [@united_states_fish_and_wildlife_service_endangered_2015].
While conservation of sagebrush ecosystems is primarily aimed at keeping sage-grouse off the endangered species list, sagebrush conservation will also benefit other species [@rowland_greater_2006; @smith_habitat_2019; @pilliod_reptiles_2020].
...
Habitat loss and transformation is a major driver of biodiversity loss [@powers_global_2019; @cardinale_biodiversity_2012; @sala_global_2000], and infrastructure development directly leads to habitat loss and transformation.
Wyoming has ideal conditions for future renewable energy development such as wind [@lloyd_prairie_2022], and while wind energy is excluded from sage-grouse core areas, achieving renewable electricity production targets would require expansion beyond disturbed lands [@kiesecker_win-win_2011].
...
A key feature of our approach is using the probabilistic projections to evaluate specific management targets for sagebrush cover in sage-grouse core areas, with implications for other sagebrush-dependent wildlife [@remington_sagebrush_2021].
...
The BIT products are estimates with associated error [@rigge_quantifying_2020], which we ignore in this analysis.
...
ggplot(bit, aes(x = year, y = mean)) +
  ...
  geom_line() +
  facet_wrap(~lab, scales = "free_y", ncol = 5) +
...
First, as mentioned above, average spring-through-summer precipitation and temperature have been identified as important covariates in previous work [@tredennick_forecasting_2016; @kleinhesselink_response_2018].
These two covariates also have the advantage of being directly tied to the limits of sagebrush performance in terms of cold, moist versus warm, dry sites [@palmquist_divergent_2021].
...
For each core area, our model represents the observed sagebrush cover in cell *i* of year *t* ($y_{i,t}$) as arising from a Poisson process with rate $\text{exp}(\mu_{i,t})$:
...
We bundled the computer code needed to format data, fit the model, evaluate the model, and project the model forward in an R package called **sageCastR** (https://github.com/WEST-Inc/sageCastR, archived on Zenodo @tredennick_zenodo_2023).
...
Projections generated during this study are available as a USGS data release [@tredennick_data_2023].
...
Convergence diagnostics indicated that most MCMC chains converged on their stationary distributions (upper 0.95 quantiles of $\hat{R}$ < 1.1 for all parameters; Appendix S1: Tables S1 - S39).
...
Most Bayesian *P*-values indicated no lack-of-fit because they were greater than 0.05 and less than 0.95 for both the spatial ($P_B^{\text{space}}$) and temporal ($P_B^{\text{time}}$) test statistic *P*-values (Appendix S1: Table S40).
...
Averaging over all the posterior distributions for the precipitation and temperature sensitivities, we found that overall average sensitivity to precipitation was negative: `r precipSummary`. 
The overall average sensitivity to temperature was positive: `r tempSummary`.
...
The finding is also consistent with results across a broader range of the sagebrush distribution using the BIT data but with more and different climate predictors [@rigge_projected_2021].
...
@palmquist_divergent_2021 showed that soil moisture will likely remain adequate under a warming climate in cold, winter-wet areas where sagebrush performance is temperature-limited.
...
Last, variable responses to above- and below-average precipitation is expected across large regions with variable soil depths [@germino_desert_2014].
...
Regional, landscape, and local gradients in average climate [@kleinhesselink_response_2018], soil conditions [@schlaepfer_ecohydrological_2012; @germino_desert_2014], and subspecies composition [@rosentreter_sagebrush_2005] can have a strong influence on sagebrush responses to weather and climate.
...
Evolutionary adaptation represents just one of many possible "slow" processes that impact our ability to make projections using short-term data that likely represent "fast" processes [@adler_matching_2020].
...
However, other factors not considered here can affect habitat quality, including herbaceous vegetation, structure, and configuration of sagebrush on the landscape, and disturbances @fedy_habitat_2014], in addition to higher temperatures [@blomberg_characteristics_2012; @lundblad_sensitivity_2022]. 
We also acknowledge that our use of thresholds to determine whether habitat was ``optimal'' for sage-grouse, while consistent with the Sage-grouse Habitat Assessment Framework [@stiver_sage-grouse_2015], may not correspond with more local trends in demography [@smith_are_2020]. 
Indeed, most core areas we classified as currently "suboptimal" had stable population trends [@edmunds_greater_2018], whereas fewer core areas currently had "optimal" levels of sagebrush cover (Fig. \@ref(fig:lambda-compares)). 
Therefore, we emphasize that while the thresholds used here were useful for a relative comparison of habitat status across core areas, studying effects of sagebrush cover on demographics such as population trends, at relevant spatial scales [@monroe_spatial_2022], would provide a more direct assessment of habitat quality for sage-grouse (and implications of climate change).
...
Even still, effectively conserving all of the core areas may not be enough to ensure sage-grouse conservation [@hannah_protected_2007; @monzon_climate_2011].
