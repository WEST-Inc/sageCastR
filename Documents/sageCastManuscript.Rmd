...
\noindent{}Data and code are provided as private-for-peer review (shared publicly in a repository: https://github.com/WEST-Inc/sageCastR). Data and code will be archived on Zenodo upon acceptance.
...
\new{Avoiding variable selection avoids some computational constratins.}
...
The posterior distributions of equilibrium cover $\left(y' = e^{\beta_1 / \left(1 - \beta_2 \right)}\right)$ for each core area contain the observed mean cover values, except for a few core areas (Fig. Sx).
...
Results from the colonization probability model used for future simulations are presented in Table Sx.
...
Continued conservation of these core areas has the potential to increase \new{the extent of the landscape with sagebrush cover exceeding the nesting and summer habitat thresholds} (Fig. \@ref(fig:nesting-targs) and Fig. Sx).
...
\new{So to could targeted modeling studies that use more climate and environmental covariates then we used here.}
