...
\noindent{}Data and code are provided as private-for-peer review (shared publicly in a repository: https://github.com/WEST-Inc/sageCastR). Data and code will be archived on Zenodo upon acceptance. Projections generated during this study will be available as a USGS data release [@tredennick_data_2023] (link to be activated upon acceptance). All data used in this study are publicly available elsewhere: sagebrush cover data, @rigge_quantifying_2020; PRISM climate product, http://prism.oregonstate.edu/; CMIP climate projections, @eyring_overview_2016 accessed with **epwshiftr** R package [@jia_epwshiftr_2020].
...
coreAreaLambdas <- read_csv("../AuxData/EdmundsLambdaEstimates.csv") %>%
  filter(a == "a") %>%
...
coreAreaSizes <- sf::st_read("../AuxData/corev4_072915_final.shp") %>%
...
Sagebrush (*Artemisia* spp.) ecosystems provide critical habitat for the Greater sage-grouse (*Centrocercus urophasianus*), \new{a species of conservation concern}.
...
For example, land managers must consider the long-term effects of climate change and the near-term impacts of ecosystem degradation [e.g., @nepstad_interactions_2008].
Because populations, communities, and ecosystems are not static, using the current status of an ecosystem \new{may be insufficient to define protected areas for species} [@hannah_protected_2007; @monzon_climate_2011] or to make optimal management decisions [@boettiger_optimal_2016].
...
Land management in the western United States focuses on several uses and species, but Greater sage-grouse (*Centrocercus urophasianus*, hereafter "sage-grouse") \new{has historically driven} the conversation in sagebrush dominated ecosystems.
...
[USFWS] 2010).
\new{A further review of population status and conservation efforts concluded the species did not warrant listing in 2015} [@united_states_fish_and_wildlife_service_endangered_2015].
\new{While} conservation of sagebrush ecosystems is primarily aimed at keeping sage-grouse off the endangered species list but sagebrush conservation will benefit other species, too [@rowland_greater_2006; @smith_habitat_2019; @pilliod_reptiles_2020].
...
Sage-grouse populations are active in all core areas and most maintain stable populations [@edmunds_greater_2018].
...
\new{Habitat loss and transformation is a major driver of biodiversity loss} [@powers_global_2019; @cardinale_biodiversity_2012; @sala_global_2000], \new{and infrastructure development directly leads to habitat loss and transformation.}
\new{Wyoming has ideal conditions for future renewable energy development such as wind} [@lloyd_prairie_2022]\new{, and while wind energy is excluded from sage-grouse core areas, achieving renewable electricity production targets would require expansion beyond disturbed lands} [@kiesecker_win-win_2011].
...
\new{However, if some areas may become less suitable for sage-grouse due to climate change, those areas might accommodate renewable energy infrastructure without long-term effects on the species.} 
...
More broadly, assessing current and future habitat suitability *vis-a-vis* species of concern is useful as we face difficult and inevitable trade-offs between land preservation and land-use to combat climate change.

The sagebrush biome is in peril [@davies_saving_2011; @remington_sagebrush_2021],
but Wyoming is thought to be a stronghold where sagebrush will persist and possibly thrive in the future, especially compared to other parts of its range [@palmquist_divergent_2021; @renwick_multi-model_2018-1].
This may partly be due to relatively cooler temperatures in Wyoming [@renwick_multi-model_2018-1] and that Wyoming has largely avoided widespread cheatgrass (*Bromus tectorum* L.) invasion compared to other parts of the sagebrush biome [@pastick_rapid_2021].
...
\new{The BIT products are estimates with associated error} [@rigge_quantifying_2020], which we ignore in this analysis.
\new{Assuming errors are not related to time or space, the impact of including error would likely increase the error of estimates in the model described below.}
...
bit <- bit %>%
  left_join(nameMap %>%
              ...
              rename("coreName" = Name)) %>%
  mutate(lab = paste0(coreName, "\n(", Abbreviation, ")"))
...
\new{PRISM data were resampled to 100-m resolution using bilinear interpolation to match the BIT data.}
...
\new{The choice to use two simple covariates is motivated by biology, modeling constraints, and the desire to avoid spurious, difficult-to-explain effects.} \new{First, as mentioned above, average spring-through-summer precipitation and temperature have been identified as important covariates in previous work} [@tredennick_forecasting_2016; @kleinhesselink_response_2018].
\new{These two covariates also have the advantage of being directly tied to the limits of sagebrush performance in terms of cold, moist versus warm, dry sites} [@palmquist_divergent_2021].
\new{Second, the models we fit are computationally intensive due to the large size of each core area and the number of core areas we modeled.}
\new{Avoiding variable selection reduces some computational constraints.}
\new{We also needed to use covariates that could be easily extracted and computed from GCM output for future projections}.
\new{Last, given the number of core areas we modeled, the use of many covariates could increase the chance of detecting spurious effects, which we sought to avoid.}
...
\new{ssp126 is a version the RCP2.6 scenario and assumes that radiative forcing reaches 2.6 W/m$^2$ in 2100.}
\new{ssp245 is a version of the RCP4.5 scenario and assumes that radiative forcing reaches 4.5 W/m$^2$ in 2100.}
\new{ssp585 is a version of the RCP8.5 scneario and assumes that radiative forcing reaches 8.5 W/m$^2$ in 2100.}
GCM ouputs were downloaded from Lawrence Livermore National Library archives of CMIP6 model simulations and projections.
We used the `esfg_query` function from the **epwshiftr** R package [@jia_epwshiftr_2020] to query the database and download the data.
\new{GCMs had to include historic, ssp126, ssp245, and ssp585 scenarios for inclusion.}
...
\new{For simulating projections, GCM data were resampled from 100-km resolution to 1-km resolution, cropped to each core area extent, and then the average covariate value across 1-km grid cells within each core area was calculated.}
\new{This means that climate covariates for projections did not vary across space within each core area, only over time.}
...
\new{Models were fit to each core area independently.}
\new{For each core area,} our model represents the observed sagebrush cover in cell *i* of year *t* ($y_{i,t}$) as arising from a Poisson process with rate $\text{exp}(\mu_{i,t})$:
...
\new{There were very few instances of 0 cover, so this work-around likely had small, if any, influence on the results.}
...
The `focal` function from the **raster** package [@hijmans_raster_2023] in R cannot accommodate missing values when calculating a focal mean with custom weights.
...
Specifically, we used the Stan software [@stan_development_team_stan_2020] to implement the HMC Markov chain Monte Carlo (MCMC) algorithm, using the rstan package [@guo_rstan_2020] to connect Stan to R.
...
We bundled the computer code needed to format data, fit the model, evaluate the model, and project the model forward in an R package called **sageCastR** (https://github.com/WEST-Inc/sageCastR).
...
\new{Projections generated during this study are available as a USGS data release} [@tredennick_data_2023].
...
We used sage-grouse population growth rate estimates from @edmunds_greater_2018 [@noauthor_erratum_2018] to see whether core areas projected to become optimal or suboptimal have currently declining, stable, or increasing sage-grouse populations.
...
Relative errors of the GCMs showed that no single GCM was best at recreating historic \new{average spring-through-summer} temperature or \new{average spring-through-summer} precipitation (Fig. \@ref(fig:gcm-ranks)A).
...
climateAll <- read.csv("../AuxData/CMIP6_future_by_core.csv") %>%
  ...
  rename("GCM Scenario" = experiment_id,
         ...
         "Avg. spr-sum Precip. (mm/day)" = MeanMean_pr,
         "Var. spr-sum Precip." = MeanVar_pr,
         "Avg. spr-sum Temp. (deg. C)" = MeanMean_tas,
         "Var. spr-sum Temp." = MeanVar_tas)
...
climate <- read.csv("../AuxData/CMIP6_future_by_core.csv") %>%
  ...
  mutate(name = case_when(name == "pr" ~ "Spr-Summ Precipitation (mm/day)",
                          TRUE ~ "Spr-Summ Temperature (deg. C)"))
...
Convergence diagnostics indicated that most MCMC chains converged on their stationary distributions (upper 0.95 quantiles of $\hat{R}$ < 1.1 for all parameters; Tables S1 - S39).
\new{This was true except for the model for Elk Basin West (Table S9).}
\new{Results from Elk Basin West are not further discussed because the MCMC chains did not converge and because the model failed the posterior predictive checks described next.}
Most Bayesian *P*-values indicated no lack-of-fit because they were greater than 0.05 and less than 0.95 for both the spatial ($P_B^{\text{space}}$) and temporal ($P_B^{\text{time}}$) test statistic *P*-values (Table S40).
...
precipSummary <- paste0(
  ...
  " (pseudo 95% Bayesian credible interval (BCI) = ",
...
Averaging over all the posterior distributions for the precipitation and temperature \new{sensitivities}, we found that overall average \new{sensitivity} to precipitation was negative: `r precipSummary`. 
The overall average \new{sensitivity} to temperature was positive: `r tempSummary`.
...
The variance of the overall average \new{sensitivities} should be viewed as the variance of the mean sensitivity, not as the variance of \new{sensitivity magnitudes} across core areas.
...
More core areas were projected to increase \new{in sagebrush cover} (n = 21 for ssp126; n = 22 for ssp245 and ssp585) than decrease (n = 13 for ssp126; n = 12 for ssp245 and ssp585) and the magnitude of increases was greater than decreases for each GCM scenario.
...
In contrast, `r as.character(round(min(areaDiffsDec$TotalArea)))` - `r as.character(round(max(areaDiffsDec$TotalArea)))`  $\text{km}^2$ of land is projected to experience decreases in sagebrush cover on average, depending on GCM scenario.
...
\new{Salt Wells is located in southwestern Wyoming} (Fig. \@ref(fig:sensitivities)).
...
At this core area, projections show sagebrush \new{increasing slightly over most of the area, but with large increases in cover in the western edge of the core area.} (Fig. \@ref(fig:spatial-projections)).
...
\new{Results based on summer habitat cover targets are similar (Fig. S41).}
...
Continued conservation of these core areas has the potential to increase \new{the extent of the landscape with sagebrush cover exceeding the nesting and summer habitat thresholds} (Figs. \@ref(fig:nesting-targs) and S41).
...
The finding is also consistent with results across a broader range of the sagebrush distribution using \new{the BIT data but with more and different climate predictors} [@rigge_projected_2021].
...
@palmquist_divergent_2021 showed that soil moisture will likely remain adequate under a warming climate in \new{cold, moist areas where sagebrush performance is temperature-limited.}
...
A negative \new{sensitivity} to precipitation combined with a positive \new{sensitivity} to temperature could reflect sagebrush responding negatively to late season snow in colder years and positively to earlier snowmelt in warmer years.
\new{The negative sensitivity of sagebrush to precipitation is confusing, but the negative sensitivity could be reflecting an interaction we failed to model.}
...
@schlaepfer_understanding_2021 \new{found a similar result in terms of sagebrush regeneration: regeneration will likely remain possible under a warming climate alone, but the fire-cheatgrass interaction will likely limit sagebrush persistence.}
Nonetheless, our results corroborate those of @palmquist_divergent_2021 and @schlaepfer_understanding_2021 by suggesting that climate change alone should benefit, or at least not significantly disadvantage, sagebrush in Wyoming.

\new{A second limitation is that we implicitly assume that the relationships between sagebrush percent cover and temperature and precipitation are log-linear.
...
It is certainly possible that sagebrush performance will plateau at temperatures and precipitation levels outside of the historical range of variability, either because of physiological limits or evolutionary adaptation} [@adler_matching_2020].
\new{Evolutionary adaptation represents just one of many possible "slow" processes that impact our ability to make projections using short-term data that likely represent "fast" processes} [@adler_matching_2020].
\new{Sagebrush are slow to recover from extreme events like drought that cause die-offs, representing another slow process potentially not captured by our models.}
\new{Therefore, our models likely miss the impact of such extreme events.}
...
\new{So too could targeted modeling studies that use more climate and environmental covariates then we used here.}
...
\new{However, other factors not considered here can affect habitat quality, including herbaceous vegetation, structure, and configuration of sagebrush on the landscape, and disturbances} @fedy_habitat_2014], \new{in addition to higher temperatures} [@blomberg_characteristics_2012; @lundblad_sensitivity_2022]. 
\new{We also acknowledge that our use of thresholds to determine whether habitat was ``optimal'' for sage-grouse, while consistent with the Sage-grouse Habitat Assessment Framework} [@stiver_sage-grouse_2015], \new{may not correspond with more local trends in demography} [@smith_are_2020]. 
\new{Indeed, most core areas we classified as currently "suboptimal" had stable population trends} [@edmunds_greater_2018] \new{whereas fewer core areas currently had "optimal" levels of sagebrush cover} (Fig. \@ref(fig:lambda-compares)). 
\new{Therefore, we emphasize that while the thresholds used here were useful for a relative comparison of habitat status across core areas, studying effects of sagebrush cover on demographics such as population trends, at relevant spatial scales} [@monroe_spatial_2022], \new{would provide a more direct assessment of habitat quality for sage-grouse (and implications of climate change).}
...
\new{Our results suggest that climate change per se is not going to cause declines in sagebrush in Wyoming.}
\new{Therefore, management actions aimed at reducing cheatgrass invasion and limiting disturbances in core areas might help mitigate the negative, indirect impacts of climate change.}
...
Even still, effectively conserving all of the core areas may not be enough \new{to ensure sage-grouse conservation} [@hannah_protected_2007; @monzon_climate_2011].
...
\new{This suggests that} maintaining large intact landscapes inside and outside of core areas (Heinrichs et al. 2019) \new{can help} ensure viable sage-grouse populations persist. 
Our models help to identify sagebrush habitats that \new{are likely to} persist into the future given climate change, \new{highlighting areas for potential long-term preservation and protection.}
